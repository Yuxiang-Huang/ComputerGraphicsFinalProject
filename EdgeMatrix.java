...
public class EdgeMatrix extends Matrix {
  ...
  double x3, double y3, 
  double z0,
  int curveType, double steps2D, int steps) {
    ...
    double[] ym = ycoefs.get(0);
     ...
     //find points on line
     for (t = steps2D; t <= 1.000001; t += steps2D) {
  ...
  int curveType, double steps2D, int steps ) {
    ...
    Matrix points = generateCurve(x0, y0, x1, y1, x2, y2, x3, y3, z0, curveType, steps2D, steps);
