...
public class GfxVector {
  ...
  public GfxVector getNormalized() {
    setMagnitude();
    setNormalized();
    return new GfxVector(normalized[0], normalized[1], normalized[2]);
  }
  ...
  public boolean equals(GfxVector v) {
    return components[0] == v.components[0] &&
      components[1] == v.components[1] &&
      components[2] == v.components[2]; 
  }//equals

  public GfxVector interpolate(GfxVector v, double t) {
    double[] temp = new double[3];
    for (int i=0; i<3; i++) {
      temp[i] = components[i] * (1-t) + v.components[i] * t;
    }
    return new GfxVector(temp[0], temp[1], temp[2]);
  }
